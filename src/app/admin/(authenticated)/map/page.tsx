...
export default function AdminMapPage() {
  const [fromInput, setFromInput] = useState(() =>
    dateToString(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)),
  );
  const [toInput, setToInput] = useState(() => dateToString(new Date()));
  const [applied, setApplied] = useState(() => ({
    from: dateToString(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)),
    ...
    <div className="space-y-4">
      ...
      {isError && (
        <div className="rounded-2xl border border-destructive/50 bg-destructive/10 px-4 py-3 text-sm text-destructive">
          {error?.message ??
            "Failed to load money flow data. Check that the API is running and GEOIP_DB_PATH is set."}
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <MoneyFlowGlobe
          points={points}
          flows={flows}
          className="h-[560px] w-full rounded-2xl overflow-hidden bg-muted/30"
        />
      ) : (
        <MoneyFlowMap
          points={points}
      ...
      {!isLoading && !isError && points.length === 0 && flows.length === 0 && (
        <div className="rounded-2xl border border-dashed border-border bg-muted/20 px-4 py-8 text-center text-sm text-muted-foreground">
          No money flow data in this date range. Locations are derived from the
          last known session IP per user. Ensure users have logged in and
          transactions exist in the selected period.
        </div>
      )}

      <p className="text-xs text-muted-foreground">
        Money movement by location (session IP). No raw IPs are shown. Requires
        backend GEOIP_DB_PATH and MaxMind GeoLite2-City.
      </p>
    </div>
  );
}
