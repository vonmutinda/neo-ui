...
export function useExports(bizId: string | null, filter?: ExportFilter) {
  ...
  return useQuery<PaginatedResult<ExportRequest>>({
    queryKey: ["business", bizId, "exports", qs],
    queryFn: () =>
...
export function useUpdateExportStatus(bizId: string | null) {
  const qc = useQueryClient();
  ...
  >({
    mutationFn: ({ exportId, body }) =>
      api.patch(`/v1/business/${bizId}/exports/${exportId}/status`, body),
...
export function useRecordProceeds(bizId: string | null) {
  ...
  return useMutation<
    unknown,
    Error,
    { exportId: string; body: { amountCents: number; currency: string } }
  >({
    mutationFn: ({ exportId, body }) =>
    ...
    onError: (err) =>
      toast.error("Failed to record proceeds", { description: err.message }),
  });
}
...
export function useRecordSurrender(bizId: string | null) {
  ...
  >({
    ...
    onError: (err) =>
      toast.error("Failed to record surrender", { description: err.message }),
  });
}
...
export function useExportDocumentUrl(
  bizId: string | null,
  ...
  return useQuery<DocumentURLResponse>({
    ...
    queryFn: () =>
      api.get<DocumentURLResponse>(
        `/v1/business/${bizId}/exports/${exportId}/documents/${docId}/url`,
      ),
    enabled: !!bizId && !!exportId && !!docId,
  });
}

export function useReviewExportDocument(bizId: string | null) {
  const qc = useQueryClient();

  return useMutation<
    { status: string },
    Error,
    { exportId: string; docId: string; body: ReviewDocumentRequest }
  >({
    mutationFn: ({ exportId, docId, body }) =>
      api.patch<{ status: string }>(
        `/v1/business/${bizId}/exports/${exportId}/documents/${docId}/review`,
        body,
      ),
    onSuccess: (_, { exportId }) => {
