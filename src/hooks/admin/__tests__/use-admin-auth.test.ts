...
import { useAdminLogin, useAdminChangePassword, useAdminMe } from "../use-admin-auth";
import {
  setupAdminAuth,
  clearAdminAuth,
  createWrapper,
...
describe("useAdminLogin", () => {
  it("calls POST /auth/login with email and password", async () => {
    ...
    result.current.mutate({ email: "admin@neo.et", password: "secret" });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expectAdminCall(globalThis.fetch, "POST", "/auth/login");
    expectAdminCallBody(globalThis.fetch, { email: "admin@neo.et", password: "secret" });
  });
});
...
describe("useAdminChangePassword", () => {
  it("calls POST /auth/change-password", async () => {
    ...
    const { result } = renderHook(() => useAdminChangePassword(), { wrapper: createWrapper() });
    result.current.mutate({ currentPassword: "old", newPassword: "new" });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expectAdminCall(globalThis.fetch, "POST", "/auth/change-password");
    expectAdminCallBody(globalThis.fetch, { currentPassword: "old", newPassword: "new" });
  });
});
...
describe("useAdminMe", () => {
  it("calls GET /staff/me with auth header", async () => {
    ...
    const { result } = renderHook(() => useAdminMe(), { wrapper: createWrapper() });
